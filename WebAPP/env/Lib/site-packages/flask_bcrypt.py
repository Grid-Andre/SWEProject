...
def generate_password_hash(password, rounds=None):
    ...
    configuration variable provided when passing back the application object. In other
    words this shortcut does not make use of the application object at all.
...
class Bcrypt(object):
    ...
    bcrypt, of course. This class may be used to intialize your Flask application
    ...
    To get started you will wrap your application's application object something like
    this::
    ...
    .. admonition:: Namespacing Issues 
        ...
        It's worth noting that if you use the format, `bcrypt = Bcrypt(application)`
        ...
        = Bcrypt(application)` to prevent naming collisions.

    Additionally a configuration value for `BCRYPT_LOG_ROUNDS` may be set in 
    the configuration of the Flask application. If none is provided this will
