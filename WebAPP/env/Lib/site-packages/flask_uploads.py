...
def patch_request_class(app, size=64 * 1024 * 1024):
    ...
    This patches the application's request class's
    ...
    :param app: The application to patch the request class of.
    :param size: The maximum size to accept, in bytes. The default is 64 MiB.
                 If it is `None`, the application's `MAX_CONTENT_LENGTH` configuration
...
def config_for_set(uset, app, defaults=None):
    ...
    :param app: The application to load the configuration from.
...
def configure_uploads(app, upload_sets):
    """
    Call this after the application has been configured. It will go through all the
    upload sets, get their configuration, and store the configuration on the
    application. It will also register the uploads module if it hasn't been set. This
...
class UploadSet(object):
    ...
    :param default_dest: If given, this should be a callable. If you call it
                         with the application, it should return the default upload
                         destination path for that application.
    """
